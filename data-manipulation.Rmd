...
Conform bba1m and bba2m so they have the same quads
```{r}
...
if (land_cover) {
  ...
  bba2m.nlcd <- st_transform(bba2m, st_crs(nlcd))
...
```{r}
#' Objective: Calculate a line boundary of a given species by averaging the 10
#' most extreme values
#' Inputs: 
#' @param col a string representing a species name
#' @param atlas a data.frame with species-specific columns
#' @param max a boolean value that is true if the objective is to find the maximum
#' @return val = the latitudinal or longitudinal value of a geographic line boundary
#' @export
...
Create a data.frame with only quads minimally covered in A1
```{r}
dim(bba1m)
dim(bba2m)
bba1m.min <- bba1m %>%
...
bba2m.min <- bba2m %>%
  filter(QuadNameCo %in% bba1m.min$NAME)
...
bba1m.dist.min <- dist_df(bba1m.min, species.a1)
bba2m.dist.min <- dist_df(bba2m.min, species.a2)
bba1m.dist.min
bba2m.dist.min
```
...
bba1m.dist.min <- merge(bba1m.dist.min, tax, by = "species")
bba2m.dist.min <- merge(bba2m.dist.min, tax, by = "species")
head(bba1m.dist.min)
head(bba2m.dist.min)
```
...
# repeat for minimum coverage data.frames
common.a1.min <- bba1m.dist.min %>%
  filter(species %in% bba2m.dist$species)

common.a2.min <- bba2m.dist.min %>%
  filter(species %in% bba1m.dist$species)
```


Create distance to coast column
This step is not repeated for the minimum coverage data.frames
```{r}
...
# repeat process for quad centroids
...
# minimum coverage
saveRDS(object = bba1m.min, file = "rds-files\\bba1m_min_cov.rds")
saveRDS(object = bba2m.min, file = "rds-files\\bba2m_min_cov.rds")
saveRDS(object = common.a1.min, file = "rds-files\\common_species1_min.rds")
saveRDS(object = common.a2.min, file = "rds-files\\common_species2_min.rds")

# misc.
saveRDS(object = bba1m.dist.min, file = "rds-files\\bba1m_distribution_min_cov.rds")
saveRDS(object = bba2m.dist.min, file = "rds-files\\bba2m_distribution_min_cov.rds")
